...
// 3.@synthesize @dynamic 。关键字的作用说明:@property 预编译命令（关键字）作用是声明属性的 getter和setter方法， @synthesize 生成属性访问器即 getter和setter方法。 @dynamic 禁止编译器生成 getter／setter，通过自定义或者在运行时动态创建绑定：主要使用在Core Data中。   atomic 是操作原子性，作为属性修饰词，则编译器生产的 getter／setter方法中存在 @synchronized(self)或者lock锁机制只允许原子性访问。 若只自定义一个属性访问器（setter／getter中一个），会出现warning：writable atomic property ‘name’ cannot pair a synthesized getter with …
// 若setter getter两个都定义，则需要 @synthesize name = _name; 两个都自定义的情况下，系统不会合成属性访问器，则此时需要显示调用 @synthesize来定义属性的实例变量名，以便使用属性的实例变量
